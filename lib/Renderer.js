...
// Paints strokes onto a Canvas 2D context at scale 1. Strokes hold global
// logical coordinates; cfg.offsetX/offsetY move them into the canvas.
//
// cfg = {
//   offsetX, offsetY   translation applied to every point
//   excludeId          stroke id to skip (drawn elsewhere while dragged/edited)
//   numberIndex        { strokeId: displayed number }, see numbering()
...
var MAX_CANVAS_FONT_PX = 48             // larger text is scaled up; Qt's glyph cache degrades above this
...
// Number markers display 1..N in creation order (by id), so removing one
...
function drawStroke(ctx, stroke, cfg) {
    ...
    if (stroke.tool === "mosaic")
        return // drawn by ShaderEffectSource items in AnnotationLayer
    ...
    case "ellipse": {
        ...
        if (e.w > 0 && e.h > 0) {
            ...
            ctx.restore()   // before stroke(), so the pen is not scaled
            ctx.stroke()
        }
        break
    }
    ...
    case "number": {
        ...
        // "middle" centers the em box, not the digits, which sit ~15% high.
...
// Dashed frame around the selected stroke: a light solid line under dark
// dashes, so it reads on any background.
